...
async def delete_document(request: Request, document_name: str) -> APIResponce:
    db_client = request.app.state.db_client
    document_model = await DocumentModel.get_instance(db_client)
    ...
    matching_files = [
        ...
        if f.is_file() and f.name.endswith(search_suffix)
    ]
...
@data.post("/ingest")
async def ingest_file(request: Request, document_id: str) -> APIResponce:
    db_client = request.app.state.db_client
    document_model = await DocumentModel.get_instance(db_client)
        ...
        DataChunk(
            ...
            chunk_metadata={
                **chunk["metadata"],
                ...
                "section_headings": chunk.get("metadata", {}).get(
                    "section_headings", []
                ),
            },
...
async def delete_document_chunks(request: Request, document_id: str) -> APIResponce:
    ...
    deleted_chunks_count = await chunk_model.delete_chunks_by_document_id(
