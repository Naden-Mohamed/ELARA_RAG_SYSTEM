...
class DocumentModel:
    ...
    async def init_collection(self):
        ...
        if DataBaseEnums.DOCUMENTS_COLLECTION.value not in all_collections:
            ...
            logger.info(
                f"Collection '{DataBaseEnums.DOCUMENTS_COLLECTION.value}' created."
            )
    ...
    async def update_status(
        ...
        if updated_fields["status"] in (
            DataBaseEnums.PROCESSED.value,
            DataBaseEnums.FAILED.value,
        ):
            updated_fields["processed_at"] = datetime.now(UTC)

        return await self.collection.update_one(
            {"_id": ObjectId(doc_id)}, {"$set": updated_fields}
        )
