...
class TestSearchRequiresAuth:
    ...
    async def test_search_without_token_is_rejected(self, client):
        resp = await client.post("/rag/search", json={"text": "pregnancy symptoms", "limit": 5})
...
class TestPushRequiresAuth:
    ...
    async def test_push_without_token_is_rejected(self, client):
        resp = await client.post("/rag/push", json={"document_id": "64b64b64b64b64b64b64b64"})
        assert resp.status_code in (401, 403)
...
class TestSearchAndRetrieval:
    ...
    async def test_search_returns_most_similar_chunk_first(
        ...
        await fake_vectordb.insert_many(
            "ELARA", texts=texts, vectors=vectors,
            metadatas=[{"doc_name": "nutrition.pdf"}, {"doc_name": "cars.pdf"}],
        )

        resp = await client.post(
            "/rag/search", headers=headers, json={"text": "pregnancy nutrition advice", "limit": 2}
        )
        body = resp.json()
...
class TestTestPromptEndpoint:
    @pytest.mark.asyncio
    async def test_test_prompt_uses_provided_context_chunks(self, client, fake_llm_service):
        resp = await client.post("/rag/test-prompt", json={
            "query": "What is BPCR?",
            "persona": "doctor",
            "language": "en",
            "context_chunks": [{
                "chunk_id": "c1", "doc_name": "who.pdf", "page_number": 1,
                "section": "Intro", "text": "BPCR stands for Birth Preparedness...",
            }],
        })
        body = resp.json()
    ...
    @pytest.mark.asyncio
    async def test_test_prompt_falls_back_to_mock_chunks_when_none_given(self, client, fake_llm_service):
        resp = await client.post("/rag/test-prompt", json={
            "query": "What is BPCR?", "persona": "doctor", "language": "en",
        })
