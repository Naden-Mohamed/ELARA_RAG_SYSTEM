...
class TestRegister:
    ...
    async def test_register_rejects_invalid_email_format(self, client):
        ...
        assert resp.status_code == 422  # pydantic validation, handled by FastAPI directly
...
class TestLogin:
    @pytest.mark.asyncio
    async def test_login_with_correct_credentials_succeeds(self, client, registered_user):
        _, _, payload = registered_user
...
class TestMe:
    ...
    async def test_me_requires_auth(self, client):
        ...
        assert resp.status_code in (401, 403)  # HTTPBearer raises 403 when header missing entirely

    @pytest.mark.asyncio
    async def test_me_returns_current_user_without_password_hash(self, client, registered_user):
