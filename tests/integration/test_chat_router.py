...
class TestSendMessage:
    ...
    @pytest.mark.asyncio
    async def test_send_creates_new_chat_when_no_chat_id_given(self, client, registered_user):
        headers, _, _ = registered_user
        resp = await client.post("/chat/send", headers=headers, json={"query": "What is BPCR?"})
        body = resp.json()
    ...
    @pytest.mark.asyncio
    async def test_send_persists_user_and_assistant_messages(self, client, registered_user):
        headers, _, _ = registered_user
        resp = await client.post("/chat/send", headers=headers, json={"query": "Hello there"})
    ...
    @pytest.mark.asyncio
    async def test_reusing_own_chat_id_continues_same_conversation(self, client, registered_user):
        headers, _, _ = registered_user
        first = await client.post("/chat/send", headers=headers, json={"query": "First message"})
        chat_id = first.json()["data"]["chat_id"]

        second = await client.post(
            "/chat/send", headers=headers, json={"query": "Second message", "chat_id": chat_id}
        )
...
class TestChatIsolationBetweenUsers:
    ...
    async def test_cannot_send_message_into_another_users_chat(
        ...
        hijack_attempt = await client.post(
            "/chat/send", headers=headers_b, json={"query": "trying to read your chat", "chat_id": chat_id}
        )
    ...
    async def test_cannot_read_another_users_chat_history(
        ...
        body = resp.json()
        assert body["status"] == "failed"
        assert "not found" in body["error"].lower() or "unauthorized" in body["error"].lower()

    @pytest.mark.asyncio
    async def test_my_chats_only_returns_own_chats(self, client, registered_user, second_user):
        headers_a, _, _ = registered_user
        headers_b, _, _ = second_user

        await client.post("/chat/send", headers=headers_a, json={"query": "A's question"})
        await client.post("/chat/send", headers=headers_b, json={"query": "B's question"})
...
class TestPagination:
    ...
    async def test_history_pagination_has_more_flag(self, client, registered_user):
        headers, _, _ = registered_user
        first = await client.post("/chat/send", headers=headers, json={"query": "msg 1"})
        chat_id = first.json()["data"]["chat_id"]
        for i in range(2, 5):
            await client.post(
                "/chat/send", headers=headers, json={"query": f"msg {i}", "chat_id": chat_id}
            )

        resp = await client.get(f"/chat/{chat_id}/history?page=1&page_size=2", headers=headers)
        body = resp.json()["data"]
