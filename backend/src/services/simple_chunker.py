...
class SimpleChunker:
    ...
    def extract_text_by_page(self, file_path: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.info(
                f"Successfully extracted and cleaned {len(pages)} pages from {file_path}"
            )
        except Exception as e:
            logger.error(f"Failed to process PDF {file_path}: {e!s}")
            raise
    ...
    def chunk(self, file_path: str) -> list[dict[str, Any]]:
        ...
        if not any(p["text"].strip() for p in pages):
            logger.warning(
                f"No extractable text found in {file_path}. Possibly a scanned document."
            )
            return []
